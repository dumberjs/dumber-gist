...
test('Au1DepsFinder find deps for npm js file', async t => {
  ...
  const f = new Au1DepsFinder(primitives);
  const deps = await f.findDeps(`${JSDELIVR_PREFIX}foo@1.0.0/dist/bar.js`, 'exports.Bar = class Bar {}');
  t.deepEqual(deps, ['text!./bar.html']);

  const deps2 = await f.findDeps(`${JSDELIVR_PREFIX}foo@1.0.0/dist/bar2.js`, 'exports.Bar = class Bar2 {}');
  t.deepEqual(deps2, []);
});
...
test('Au1DepsFinder find deps for scoped npm js file', async t => {
  ...
  const f = new Au1DepsFinder(primitives);
  const deps = await f.findDeps(`${JSDELIVR_PREFIX}@scoped/foo@1.0.0/dist/bar.js`, 'exports.Bar = class Bar {}');
  t.deepEqual(deps, ['text!./bar.html']);

  const deps2 = await f.findDeps(`${JSDELIVR_PREFIX}@scoped/foo@1.0.0/dist/bar2.js`, 'exports.Bar = class Bar2 {}');
  t.deepEqual(deps2, []);
});
...
test('Au1DepsFinder find deps for npm html file', async t => {
  ...
  const f = new Au1DepsFinder(primitives);
  const deps = await f.findDeps(`${JSDELIVR_PREFIX}foo@1.0.0/dist/foo.html`, '<template><require from="./a.css"><require from="./b"></require></template>');
